'use client';
/**
 * org-guide ブロックの表示・編集UI（② UI担当）
 *
 * - `OrgGuideRender`: 組織（部署）案内。各部署を name / description? のカードで
 *   提示し、href があれば全体を `next/link` のリンクにする（無ければ静的カード）。
 *   1/2 カラムのレスポンシブグリッド。
 * - `OrgGuideEditor`: heading と departments（name / description? / href?）の
 *   追加・削除・編集UI。
 *
...
import type { OrgGuideProps } from '@/lib/blocks/definitions';

type Department = OrgGuideProps['departments'][number];

/** 部署カードの中身（name + 任意 description）。リンク有無で外枠を切り替える。 */
const DepartmentBody: React.FC<{ dept: Department; linked: boolean }> = ({
  dept,
  linked,
}) => (
  <>
    <span className="flex items-start justify-between gap-3">
      <span
        className={`font-heading text-lg font-semibold leading-snug ${
          linked ? 'text-link group-hover:underline underline-offset-2' : ''
        }`}
      >
        {dept.name}
      </span>
...
/* =============================================================================
   公開表示（Render）
   ============================================================================ */
export const OrgGuideRender: React.FC<{ props: OrgGuideProps }> = ({ props }) => {
  const { heading, departments } = props;

  return (
    <section
      data-block="org-guide"
      aria-labelledby="org-guide-heading"
      className="bg-background text-foreground"
    >
      <div className="mx-auto max-w-5xl px-6 py-10 md:py-14">
        <h2
          id="org-guide-heading"
        ...
        {departments.length === 0 ? (
          ...
          <ul className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
            {departments.map((dept, i) => {
              const linked = Boolean(dept.href);
              return (
                <li key={i} className="flex">
                  {linked ? (
                    ...
                    >
                      <DepartmentBody dept={dept} linked />
                    </Link>
                  ) : (
                    <div className="flex w-full flex-col rounded-lg border border-border bg-surface text-surface-foreground p-5 shadow-sm">
                      <DepartmentBody dept={dept} linked={false} />
                    </div>
                  )}
                </li>
              );
...
export const OrgGuideEditor: React.FC<{
  props: OrgGuideProps;
  onChange: (next: OrgGuideProps) => void;
}> = ({ props, onChange }) => {
  const updateDept = (index: number, patch: Partial<Department>) => {
    const departments = props.departments.map((d, i) =>
      i === index ? { ...d, ...patch } : d,
    );
    onChange({ ...props, departments });
  };

  const addDept = () => {
    const departments: Department[] = [
      ...props.departments,
      { name: '新しい部署', description: '', href: '' },
    ];
    onChange({ ...props, departments });
  };

  const removeDept = (index: number) => {
    const departments = props.departments.filter((_, i) => i !== index);
    onChange({ ...props, departments });
  };

  return (
    <div data-block-editor="org-guide" className="space-y-5">
      <div className="space-y-1.5">
        <label
          htmlFor="org-guide-heading-input"
          className="font-sans text-sm font-semibold block text-foreground"
        >
          セクション見出し
        </label>
        <input
          id="org-guide-heading-input"
          type="text"
          value={props.heading}
          placeholder="例: 組織のご案内"
      ...
      <fieldset className="space-y-3 rounded-lg border border-border p-4">
        <legend className="font-sans text-sm font-semibold px-1 text-foreground">
          部署（{props.departments.length}件）
        </legend>

        {props.departments.length === 0 ? (
          <p className="font-sans text-sm text-muted-foreground">
            部署はまだありません。下のボタンで追加できます。
          </p>
        ) : (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button
                    ...
                    aria-label={`部署 ${i + 1}「${dept.name}」を削除`}
                ...
                <div className="space-y-1.5">
                  <label
                    htmlFor={`org-description-${i}`}
                    className="font-sans text-sm font-semibold block text-foreground"
                  >
                    説明（任意）
                  </label>
                  <textarea
                    id={`org-description-${i}`}
                    value={dept.description ?? ''}
                    rows={2}
                    placeholder="部署の役割や所管業務"
                    onChange={(e) => updateDept(i, { description: e.target.value })}
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    type="text"
                    value={dept.href ?? ''}
                    placeholder="例: /soshiki/shimin-ka"
                    onChange={(e) => updateDept(i, { href: e.target.value })}
                    className={inputClass}
                  />
                </div>
              </li>
